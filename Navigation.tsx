...
function KraftsportStack() {
    return (
        ...
        >
            <Stack.Screen
                name={EAppPaths.KRAFTSPORT_HOME}
                component={KraftsportScreen}
                options={{title: "Kraftsport", headerTitleStyle: globalStyles.title}}
            />
            <Stack.Screen
                name={EAppPaths.KRAFTSPORT_GRUPPE_WAEHLEN}
                component={KraftsportGruppeWaehlenScreen}
                options={{title: "Gruppe wählen", headerTitleStyle: globalStyles.title}}
            ></Stack.Screen>
            <Stack.Screen
                name={EAppPaths.KRAFTSPORT_UEBUNGEN}
                component={KraftsportUebungenScreen}
                options={({route}) => {
            ...
            ></Stack.Screen>
            <Stack.Screen
                name={EAppPaths.KRAFTSPORT_STATISTIK}
                component={KraftsportStatistikScreen}
                options={{
...
function AusdauerStack() {
    return (
        ...
        >
            <Stack.Screen
                name={EAppPaths.AUSDAUER_HOME}
                component={AusdauerScreen}
                options={{title: "Ausdauer", headerTitleStyle: globalStyles.title}}
            />
            <Stack.Screen
                name={EAppPaths.AUSDAUER_EINTRAG}
                component={NeuerAusdauerEintragScreen}
                options={({route}) => ({
                    title: route.params.item ? "Eintrag bearbeiten" : "Neuer Eintrag",
                    headerTitleStyle: globalStyles.title,
                })}
            ></Stack.Screen>
            <Stack.Screen
                name={EAppPaths.AUSDAUER_STATISTIK}
                component={AusdauerStatistikScreen}
                options={{title: "Ausdauer Statistik", headerTitleStyle: globalStyles.title}}
            ></Stack.Screen>
        </Stack.Navigator>
    );
}
...
export default function Navigation() {
    return (
        <NavigationContainer>
            <Tab.Navigator
                initialRouteName={EAppPaths.HOME}
                screenOptions={({route}) => {
                    return {
                        tabBarIcon: ({focused, size, color}) => {
                            let icon;
                            if (route.name === EAppPaths.KRAFTSPORT_STACK)
                                icon = focused ? "barbell" : "barbell-outline";
                            if (route.name === EAppPaths.AUSDAUER_STACK)
                                icon = focused ? "body" : "body-outline";
                            if (route.name === EAppPaths.HOME)
                                icon = focused ? "home" : "home-outline";
                            if (route.name === EAppPaths.SETTINGS)
                                icon = focused ? "settings" : "settings-outline";
                            // @ts-ignore
                            return <Icon.Ionicons name={icon} size={size} color={color} />;
                        },
                        tabBarActiveTintColor: primary,
                        tabBarInactiveTintColor: textColorMuted,
                        tabBarStyle: {
