...
export default function KraftsportLineChartListItem({uebung, isNotListElement}: IKraftsportLineChartListItemProps){
    ...
    return (
        ...
        <ScrollView horizontal={true}>
            <VictoryChart width={Math.max(360, uebung.data.length * 75)} height={220} domain={{ y: [0, Math.max(8, maxYValue) + 2] }}>
                <Svg height="100%" width={Math.max(360, uebung.data.length * 75)} style={{ position: "absolute" }}>
                    ...
                    <Rect width={Math.max(360, uebung.data.length * 75)} height="100%" fill="url(#grad)" rx="20" ry="20" />
                </Svg>
                <VictoryAxis
                    style={{
