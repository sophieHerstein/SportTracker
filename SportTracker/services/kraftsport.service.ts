...
export class KraftsportService {
    async fetchKraftsportData() {
        return DatabaseService.getAll(`
            SELECT t.id AS training_id, t.datum, mg.name AS muscle_group, e.name AS exercise, es.id AS exercise_set_id, es.weight, es.repetitions 
            FROM training t 
    ...
    async deleteTrainingWithId(id: string) {
        return DatabaseService.runBatch(
            [
                {
                    query: `DELETE FROM exercise_set WHERE exercise_training_id IN (SELECT id FROM exercise_training WHERE training_id = ${id})`,
                    params: [id]
                },
                {
                    query: `DELETE FROM exercise_training WHERE training_id = ${id}`,
                    params: [id]
                },
                {
                ...
                {
                    query: `DELETE FROM exercise_training WHERE training_id NOT IN (SELECT id FROM training)`
                }
            ]
        )
    }
    ...
    async getMuscleGroupData() {
        return DatabaseService.getAll(`SELECT * FROM muscle_group`)
    }

    async addMuscleGroup(name: string) {
        return DatabaseService.run(`INSERT INTO muscle_group (name) VALUES ('${name}')`)
    }

    async getEntwicklungGewichtDataForUebung(id: number) {
        return DatabaseService.getAll(`SELECT t.datum, MAX(es.weight) AS max_weight FROM exercise_set es JOIN exercise_training et ON es.exercise_training_id = et.id JOIN training t ON et.training_id = t.id JOIN exercise e ON et.exercise_id = e.id WHERE e.id = '${id}' GROUP BY t.datum ORDER BY t.datum DESC`)
    }
    ...
    async getIdForUebung(name: string) {
        return DatabaseService.getOne(`SELECT id FROM exercise WHERE name = '${name}'`)
    }
