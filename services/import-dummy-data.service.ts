        ...
        try {
            for (const line of lines) {
                ...
                if (match) {
                    ...
                    const muscleGroupResult: {id: number} | null | undefined =
                        await DatabaseService.getOne(
                    ...
                    if (!muscleGroupResult) {
                        console.error(
                            `❌ Fehler: Konnte die Muskelgruppen-ID für '${muscleGroup}' nicht abrufen.`
                        );
                        continue;
                    }
                    ...
                    const existingTraining: {id: number} | null | undefined =
                        await DatabaseService.getOne(
                    ...
                    if (existingTraining) {
                        console.log(
                            `⚠️ Training für '${muscleGroup}' am ${dateStr} existiert bereits.`
                        );
                    ...
                    console.log(
                        `✅ Neues Training für '${muscleGroup}' am ${dateStr} gespeichert.`
                    );
                }
                ...
                if (match) {
                    ...
                    await DatabaseService.run("INSERT OR IGNORE INTO exercise (name) VALUES (?);", [
                        exerciseName,
                    ]);

                    const exerciseResult: {id: number} | null | undefined =
                        await DatabaseService.getOne("SELECT id FROM exercise WHERE name = ?;", [
                            exerciseName,
                        ]);

                    if (!exerciseResult) {
                        console.error(
                            `❌ Fehler: Konnte die Übung-ID für '${exerciseName}' nicht abrufen.`
                        );
                        continue;
                    }
                    ...
                    const existingExerciseTraining: {id: number} | null | undefined =
                        await DatabaseService.getOne(
                            "SELECT id FROM exercise_training WHERE training_id = ? AND exercise_id = ?;",
                            [trainingId, exerciseId]
                        );

                    let exerciseTrainingId = existingExerciseTraining
                    ...
                    if (!exerciseTrainingId) {
                        ...
                        console.log(
                            `✅ Übung '${exerciseName}' zu Training '${trainingId}' hinzugefügt.`
                        );
                        ...
                        for (const [, weight, reps] of setsStr.matchAll(setRegex)) {
                            ...
                            console.log(
                                `✅ Satz ${weight}kg x ${reps} für '${exerciseName}' hinzugefügt.`
                            );
                        }
                    } else {
                        console.log(
                            `⚠️ Übung '${exerciseName}' ist bereits im Training enthalten.`
                        );
                    }
                }
            }
